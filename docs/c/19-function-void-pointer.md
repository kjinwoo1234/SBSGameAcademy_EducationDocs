...
용어 설명:
- 콜백(callback)은 "함수를 다른 함수에 전달해, 필요할 때 나중에 호출하는 방식"을 의미합니다.
...
연습문제:
1. 문제 1 - 함수 포인터 배열 확장
   - 문제: `add`, `sub` 외에 `mul`, `div`를 추가해 4개 연산을 함수 포인터 배열로 처리하세요.
   - 입력: 정수 2개
   - 출력: 각 연산 결과
   - 조건: 0으로 나누기 예외 처리
2. 문제 2 - 명령행 인자 출력
   - 문제: `argc`, `argv`를 이용해 전달된 모든 인자를 순서대로 출력하세요.
   - 입력: 실행 시 전달하는 문자열 인자들
   - 출력: 인자 개수와 인자 목록
   - 힌트: `for (i = 0; i < argc; i++)`
