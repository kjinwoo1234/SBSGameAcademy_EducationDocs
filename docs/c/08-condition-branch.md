...
레이블(label)이란?
- 레이블은 코드에서 "점프할 위치 이름"입니다.
- 문법은 `이름:` 형태이며, `goto 이름;`으로 그 위치로 이동할 수 있습니다.
...
`fall-through`란?
- `switch`에서 `break`를 쓰지 않아 다음 `case` 코드까지 연속 실행되는 현상입니다.
- 의도한 경우가 아니라면 버그 원인이 되기 쉬우므로, 각 `case` 끝에 `break`를 넣는 습관을 권장합니다.
...
코드 해석:
- `if` 구문은 점수 범위(90+, 80+, 그 외)를 위에서 아래로 검사합니다.
- `switch`는 `menu` 값 하나를 기준으로 `case`를 선택합니다.
- 각 `case` 뒤 `break`는 해당 분기 종료 역할을 합니다.
