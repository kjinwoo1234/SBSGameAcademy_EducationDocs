...
| `%s` | 문자열(`"Kim"` 또는 `char text[]` 같은 문자 나열) | 글자들을 이어서 출력 | `printf("%s", "Kim");` -> `Kim` |
| `%%` | 없음 | `%` 문자 자체 출력 | `printf("진행률 80%%");` -> `진행률 80%` |

`%s`는 **문자열**(글자가 여러 개 이어진 것)을 찍을 때 씁니다. `"Kim"`처럼 따옴표로 묶인 글자 묶음이 그 예입니다. `char text[] = "Kim";`처럼 변수에 담을 수도 있는데, `[]`는 “문자 칸을 여러 개 이어 둔다”는 표시입니다. 배열의 세부는 뒤 장에서 다룹니다. 컴퓨터는 문자열 끝에 **끝 표시**로 `\0`(널 문자, “여기서 문자열 끝”)을 두고, `%s`는 그 앞까지만 출력합니다. `char*`처럼 `*`가 붙는 형태는 뒤 챕터에서 배웁니다. 지금은 `"문자열"`과 `%s` 짝만 기억하면 됩니다.
...
`scanf` 매핑 표 예시 코드는 다음과 같습니다. 이 장에서는 **형식과 `&`를 맞추는 것**에 집중합니다. 입력이 몇 개 성공했는지 검사하는 문법은 **조건문** 장에서 이어갑니다.
...
아래 예시는 이름·나이·키를 한 줄로 받는 패턴입니다. `char name[20]`은 문자 칸 20개짜리 자리이고, `%19s`는 최대 19글자까지만 받아 마지막 한 칸을 끝 표시(`\0`)에 남겨 두려는 뜻입니다. `age`와 `height`는 값이 들어갈 위치가 필요하므로 `&`를 붙이고, `name`은 배열 이름이 시작 위치처럼 동작하므로 `&` 없이 넘깁니다. `printf("키: %.1f\n", height);`는 실수를 소수 첫째 자리까지 보여 줍니다.
