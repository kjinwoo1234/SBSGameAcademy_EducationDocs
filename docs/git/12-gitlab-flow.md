...
- **MR(Merge Request, 머지 리퀘스트)**가 GitHub의 PR과 같은 자리에 선다는 것을 말로 정리한다.
...
아래 그림은 **환경·단계**를 브랜치로 표현하는 GitLab Flow의 흐름 개략입니다(팀마다 브랜치 이름은 다를 수 있음).
...
GitLab에서는 PR과 같은 역할을 **MR(Merge Request, 머지 리퀘스트)**이라고 부릅니다. **저장소에 쓰기 권한이 있으면** 같은 원격에 브랜치를 올리고 MR을 열고, **오픈소스처럼 쓰기 권한이 없으면** 포크에서 MR을 보내는 흐름은 GitHub와 같습니다. 팀 내부에서는 **fork 없이** 브랜치 + MR만으로도 GitLab Flow를 운영하는 경우가 많습니다. 외부 기여가 늘면 [Chapter 14](./14-branch-comparison-fork-team-pr.md)의 포크·`upstream` 설명과 맞춰 읽으면 좋습니다.
