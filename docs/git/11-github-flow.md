...
아래 그림은 **기본 브랜치**(도식의 `master` — 실무에서는 보통 `main`)에서 **기능 브랜치**를 내어 작업하고, PR·리뷰 뒤에 다시 합치며 배포까지 이어지는 흐름을 한눈에 보여 줍니다.
...
**자주 하는 실수**로, 서로 **관련 없는 수정**(예: 오타 수정과 기능 추가)을 **같은 브랜치**에 계속 쌓으면 PR이 비대해지고 리뷰어가 피드백하기 어렵습니다. 한쪽 변경이 지연될 때 다른 쪽까지 **병합이 막히는** 일도 잦으므로, **변경 묶음마다 브랜치를 나누는** 편이 안전합니다. 변경을 **커밋하고 원격에 푸시**하면 작업이 **원격에 백업**되고, 다른 PC에서 이어 하거나 협력자가 **같은 브랜치**를 볼 수 있습니다. 피드백을 받을 준비가 될 때까지 **같은 브랜치에 추가 커밋·푸시**를 반복해도 됩니다.
