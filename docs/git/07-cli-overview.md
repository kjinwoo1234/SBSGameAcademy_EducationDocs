...
**CLI**는 **Command Line Interface**의 약자이며, 한국어로는 “명령줄 인터페이스”입니다. 텍스트 명령으로 프로그램을 조작하는 방식으로, **스크립트 자동화**, **서버 SSH 환경**, **상세 옵션 제어**에는 GUI보다 유리한 경우가 많습니다.
...
Windows에서는 **Git for Windows**에 포함된 **Git Bash**가 흔합니다. `pwd`, `ls`, `cd 폴더`로 위치를 옮기며, 경로에 공백이 있으면 따옴표로 감쌉니다.
...
**코드 해석.** `diff`는 스테이징 전·후 차이를 볼 때 쓰고, `add -p`는 변경 중 **일부만 골라 스테이징**할 때 유용합니다.
...
CLI는 **자동화·정밀 제어**에 강하고, 원격 작업은 **`remote` 등록**과 **`fetch` / `pull` / `push`의 역할 구분**으로 이해하면 됩니다.
