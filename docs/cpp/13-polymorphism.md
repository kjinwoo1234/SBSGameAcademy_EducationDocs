...
- 예제의 `virtual ~Enemy() = default;`에서 **`= default`**는 “소멸자 본문을 내가 `{}`로 직접 쓰지 않고, **컴파일러가 기본 소멸자**를 만들어 달라”는 뜻입니다. `virtual ~Enemy() {}`처럼 빈 본문을 쓰는 것과 비슷한 역할이지만, “기본 동작 그대로”를 코드에 분명히 적는 표기입니다.
...
| `= default` | 생성자/소멸자 등을 컴파일러 기본 구현으로 두겠다는 표기 |
