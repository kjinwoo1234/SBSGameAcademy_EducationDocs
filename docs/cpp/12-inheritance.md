...
- **`protected`**는 그 중간입니다. **클래스 밖**(예: `main`)에서는 `private`처럼 직접 못 보지만, **파생 클래스의 멤버 함수 안**에서는 기반의 `protected` 멤버에 접근할 수 있습니다. 아래 예에서 `hp_`를 `protected`로 둔 이유는, 나중에 파생 클래스가 HP를 다루게 할 여지를 남기면서도 `main`에서 `w.hp_`처럼 함부로 만지지 못하게 하기 위해서입니다. (이 짧은 예는 `getHp()`만 쓰므로, “잠금 강도: public < protected < private” 그림만 잡아도 됩니다.)
...
| `protected` | 밖에서는 숨기고, 파생 클래스 안에서는 쓸 수 있는 접근 지정자 |
